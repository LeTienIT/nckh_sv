        ...
        private void btnThem_Click(object sender, EventArgs e)
        {
            ...
            if(t!="")
            {
                ...
                if (kq > 0)
                {
                    ...
                    string type = "UPDATE";
                    string detail = "Username " + user.username + ", Cập nhật giảng viên tên: " + this.ten + ", khoa: " + this.khoa;
        ...
        private void btnBack_Click(object sender, EventArgs e)
        {
            if (MessageBox.Show("Xóa giảng viên.Chỉ có thể xóa khi tất cả các project liên quan đã xóa", "Cảnh bảo", MessageBoxButtons.YesNo, MessageBoxIcon.Warning) == DialogResult.Yes)
            {
                ...
                if (kq > 0)
                {
                    ...
                    string type = "DELETE";
                    string detail = "Username " + user.username + ", Xóa giảng viên tên: " + this.ten + ", khoa: " + this.khoa;
                    new database().ActivityLog(type, detail);
                    this.Close();
                }
                else
