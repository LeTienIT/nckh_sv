...
namespace ql_spkh.Model
{
    ...
    {
        private string chuoiKN = @"Data Source=DESKTOP-M8E1KAJ\MSSQLSERVER01;Initial Catalog=ql_da_nckh;Integrated Security=True";
        ...
        public void ActivityLog(string type,string detail)
        {
            ...
            {
                conn.Open();
                string sql = "INSERT INTO activitylog VALUES("+user.userid+",N'" + type + "',getdate(),N'" + detail + "')";
                cmd = new SqlCommand(sql, conn);
                cmd.ExecuteNonQuery();
            }
            catch (Exception ex)
